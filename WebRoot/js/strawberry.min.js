...
var ControlTool = com.xjwgraph.ControlTool = function(b) {
	...
	d(a.inputImgSrc);
	a.inputCode = $id("inputCode");
	d(a.inputCode, true)
	a.inputOrder = $id("inputOrder");
	d(a.inputOrder, true)
};
ControlTool.prototype = {
	submit : function() {
		...
		var l = $id("module" + N.indexId), e = l.style, 
			k = $id("backImg"+ N.indexId), m = k.style, 
			n = $id("content" + N.indexId), F = n.style, 
			B = $id("title"+ N.indexId), L = B.style, 
			p = parseInt(l.offsetWidth), A = parseInt(l.offsetHeight), z = parseInt(e.top), H = parseInt(e.left), 
			u = parseInt(k.offsetWidth), b = parseInt(k.offsetHeight), y = parseInt(m.top), s = parseInt(m.left), 
			M = parseInt(n.offsetWidth), I = parseInt(n.offsetHeight), o = parseInt(F.top), x = parseInt(F.left), 
			r = l.getAttribute("code"), xz = l.getAttribute("order"), J = k.src, E = B.innerHTML, w = a.modeTool;
		var f = new com.xjwgraph.UndoRedoEvent(function() {
			...
			if (r) {
				l.setAttribute("inputCode", r)
			}
			if (xz) {
				l.setAttribute("inputOrder", xz)
			}
			w.showPointer(l);
		...
		l.setAttribute("code", N.inputCode.value);
		l.setAttribute("order", N.inputOrder.value);
		w.isModeCross(l);
		w.changeBaseModeAndLine(l, true);
		B.innerHTML = "";
		w.flip(w.getModeIndex(l), N.inputTitle.value);
		a.smallTool.drawMode(l);
		var i = parseInt(l.offsetWidth), h = parseInt(l.offsetHeight), q = parseInt(e.top), v = parseInt(e.left), c = parseInt(k.offsetWidth), P = parseInt(k.offsetHeight), g = parseInt(m.top), d = parseInt(m.left), t = k.src, D = parseInt(n.offsetWidth), C = parseInt(n.offsetHeight), K = parseInt(F.top), O = parseInt(F.left), G = N.inputCode.value, XZ = N.inputOrder.value, j = N.inputTitle.value;
		f.setRedo(function() {
			e.width = i + "px";
			e.height = h + "px";
			e.top = q + "px";
	...
	print : function(d) {
		...
		a.inputCode.value = f.getAttribute("code")||"";
		a.inputOrder.value = f.getAttribute("order")||0;
	}
};
com.xjwgraph.Global = {};
function $id(a) {
...
LineXML.prototype = {
	...
	view : function() {
		...
		//f.baseLineIdIndex = parseInt(g.id.substring(4))
	}
};
...
BeanXML.prototype = {
	...
	viewTextXml : function(b) {
		...
		var c = window.open("saveXml.html", "", ""), a = 0, d = [];
		d[a++] = "<html>";
		d[a++] = "<head>";
...
var Utils = com.xjwgraph.Utils = {
	...
	setModel : function() {
		return this.global.controlTool.submit();
	},
	saveXml : function() {
