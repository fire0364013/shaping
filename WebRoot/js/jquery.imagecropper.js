    ...
    $.fn.extend({
        imageCropper: function(options) {
            ...
            var defaults = {
                ...
                //工作区大小
                cropWorkAreaSize: { width: 500, height: 500 },
                //截取框相对于工作区的位置
    ...
    imageCropper.prototype = {
       ...
       resetPic:function(){
            ...
            var tempSize = {
                ...
                //如果截取框在工作区中居中，则重新设置截取框的位置~~此处是电子签章的截图的截取框相对于弹出窗的位置，此处是电子签章自己的方法resetPic() ,故此处事自己计算的高度~~wjy
            if (this.settings.cropFrameRect.center) {
                this.settings.cropFrameRect.left = 150/ 2;
                this.settings.cropFrameRect.top = 250/ 2;
            }
...
//            $('.foreground-img,.background-img', this.wrapper).attr('src', this.settings.imagePath);
            ...
            this.zoom(this.img.width * this.settings.zoomLevel, this.img.height * this.settings.zoomLevel);
            //alert("图片宽度："+this.img.width+"缩放级别："+this.settings.zoomLevel+"路径："+this.settings.imagePath);
        }
        ,
