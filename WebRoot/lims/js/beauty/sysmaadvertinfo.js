			...
			.datagrid(
					{
						...
						striped : true,
						collapsible : true,
						url:rootPath + '/beauty/sysmaadvertinfo/advertinfo!toList.action',
						fit : true,
						fitColumns : true,
						scrollbarSize : 0,
						pageSize : 20,
...
function loadFile(name){
	...
	var urlParm=rootPath +'/beauty/sysmaadvertinfo/advertinfo!downLoad.action?path='+encodeParm+"&flg="+"0";
	$.ajax({
		type:"POST",
		url:urlParm,
		success:function(data){
			cache:false;
		if(data=="success"){
			var urlParmDown=rootPath +'/beauty/sysmaadvertinfo/advertinfo!downLoad.action?path='+encodeParm+"&flg="+"1";
...
function delAll() {
	...
	if (selected != null && selected != "") {
		if (window.confirm("是否删除？")) {
			...
			$.post(rootPath + "/beauty/sysmaadvertinfo/advertinfo!betchDelete.action?id="+ selcheck+"&attfilname="+encodestr, function(del) {
				if (del == 'success') {
					alert("删除成功!");
...
function addWin(){
	...
	var url = rootPath + "/beauty/sysmaadvertinfo/advertinfo!input.action";
		...
		buttons:[{
			...
			handler:function(){
				$("#attFrame",top.document.body).contents().find("#attform").form('submit',{
					url:rootPath +'/beauty/sysmaadvertinfo/advertinfo!save.action',
					onSubmit:function(){
