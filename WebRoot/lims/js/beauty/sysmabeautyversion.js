function initDataGrid(){
				...
				striped: true,
				collapsible:true,
				url : rootPath + '/beauty/sysmabeautyversion/beautyversion!toList.action',
				fit : true,
				fitColumns : true,
				scrollbarSize:0,
		...
		function addWin(id){
			...
			var url;
			if(id!=""){
			   url = rootPath +"/beauty/sysmabeautyversion/beautyversion!input.action?id=" + id;
			}else{
			  url = rootPath +"/beauty/sysmabeautyversion/beautyversion!input.action";
			}
		...
		buttons : [
				{
					...
					$("#beautyversionFrame",top.document.body).contents().find("#addbeautyversionForm").form('submit',{
						url : rootPath + "/beauty/sysmabeautyversion/beautyversion!save.action",
						onSubmit:function(){
		...
		function del(id,rowIndex){
			...
			{
				$.post( rootPath +"/beauty/sysmabeautyversion/beautyversion!delete.action",{id:id},function(del)
					{
						if(del=='success')
						{
							$('#beautyversiongrid').datagrid('clearSelections');
		...
		function delAll(){
			...
			if(selected==null || selected.length< 1){
				...
				if(window.confirm('是否删除？')){
					...
					$.post(rootPath +"/beauty/sysmabeautyversion/beautyversion!deleteMany.action",{"ids":cc},function(del){
						if(del=='success'){
							$('#beautyversiongrid').datagrid('clearSelections');
							$("#beautyversiongrid").datagrid('reload');
							alert('成功');
		...
		function detail(id,rowindex){
			$('#beautyversiongrid').datagrid('clearSelections');
			var url =  rootPath +"/beauty/sysmabeautyversion/beautyversion!view.action";
			if(id!=""){
				url = url + "?id="+id;
			}
