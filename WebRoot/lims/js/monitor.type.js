$(function(){
	var documentWidth=document.body.clientWidth;
			$('#monitordata').datagrid({
				...
				striped: true,
				collapsible:true,
				url:rootPath +'/monitortype/monitortype!toList.action', 
				fit:true,
				fitColumns : true,
				scrollbarSize:0,
				remoteSort: false,
		...
		function addWin(id){
			$('#monitordata').datagrid('clearSelections');
			var url = rootPath +"/monitortype/monitortype!input.action";
			if(id!=""){
				url = url + "?id="+id;
			}
			...
			_dialog.dialog({
				...
				buttons:[{
					...
					handler:function(){
							...
							url:rootPath +"/monitortype/monitortype!save.action",
							onSubmit:function(){
		...
		function detail(id){
			$('#monitordata').datagrid('clearSelections');
				var url = rootPath +"/monitortype/monitortype!view.action?id="+id;
				//if(id!=""){
				//	url = url + "?id="+id;
				//}
				//$(window.top.document).find("#btnProjectInfo").click();
		...
		function del(uid,row){
			...
			$('#monitordata').datagrid('selectRow',row);
				if(window.confirm('是否删除？')){
					$.post(rootPath +"/monitortype/monitortype!deleteOnlyOne.action",{id:uid},function(del){
						if(del=='success'){
							$('#monitordata').datagrid('clearSelections');
							$("#monitordata").datagrid('reload');
							alert('成功');
		...
		function delAll(){
			...
			if(selected==null || selected.length< 1){
				...
				if(window.confirm('是否删除？')){
					...
					$.post(rootPath +"/monitortype/monitortype!deleteAll.action",{"ids":cc},function(del){
						if(del=='success'){
							$('#monitordata').datagrid('clearSelections');
							$("#monitordata").datagrid('reload');
							alert('成功');
