...
function initMonitorinfoTree(flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath +"/sampletest/sample/sampletestbysample!toMonitorinfoDataAuditTree.action?projectid="+projectid;
	}		
	$('#monitorinfolist').datagrid({
			...
			columns:[[
						...
						{field:'monitorname',title:'监测点',width:120,align : 'center'},
			...
			onClickRow:function(rowIndex,rowData){
				...
				if(data!=null && data.rows.length>0){
					setTimeout("mergeCellsByField(\"monitorinfolist\",\"projectdetail,entname\")",10)
				}
				if(flag=='flag'){
					initSampleinfo('','',flag);
				}else{
...
function initSampleinfo(projectcode,monitorpointid,flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath +'/sampletest/sample/sampletestbysample!toSampleDataAudit.action?projectcode='+encodeURI(projectcode)+'&monitorpointid='+encodeURI(monitorpointid);
	}
...
function dataAudit(){
	...
	if(samplecode!=""){
		var url = rootPath + "/sampletest/sample/sampletestbysample!toDataAudit.action";
...
function initIteminfo(projectcode,monitorpointid,samplecode,flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath +'/sampletest/sample/sampletestbysample!toItemfoParameterAudit.action?samplecode='+samplecode+'&projectcode='+projectcode+'&monitorpointid='+monitorpointid;
	}
	$('#itemlist').datagrid({
		nowrap: false,
		striped: true,
...
//			itemids += rows[i].itemid;
	...
	var url = rootPath + '/sampletest/sample/sampletestbysample!toDataEntry3.action?samplecode='+samplecodeVal2+'&deviceid='+deviceid+'&sampleitemtestids='+sampleitemtestids+'&timeStamp='+new Date().getTime();
...
//流程提交
function fuHeYangPinTiJiao111111(){
	var rows = $('#itemlist').datagrid('getSelections');
	...
	if(rows!=null && rows.length>0){
		...
		$.post(
			rootPath + "/sampletest/sample/sampletestbysample!setAllStatusByAudit.action",	
...
function checkFuntion(temp){	
	...
	var str =encodeURI(encodeURI("原始数据按样品签发退回意见")); 
