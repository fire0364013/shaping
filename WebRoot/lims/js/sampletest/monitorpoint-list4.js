...
function initMonitorinfoTree(flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath +"/sampletest/originalrecordsapprove/originalrecordsapprove!toMonitorinfoSampleTestTree.action?projectid="+projectid;
	}		
...
//						{field:'pointcode',title:'监测点编码', width:90,align:"center"},
						{field:'deptname',title:'采样科室 ', width:100,align:"center"},
			...
			onClickRow:function(node){
			    ...
			    //给项目监测点id赋值
			    projectpointidVal = node.projectpointid;
			    if(node.projectpointid!=undefined && node.projectcode!=undefined){
					initSampleinfo(node.projectcode,node.monitorpointid,'');
				}else{
...
function initMonitorinfo(){
	$('#monitorinfolist').datagrid({
		...
		striped: true,
		collapsible:true,
		url:rootPath +"/sampletest/originalrecordsapprove/originalrecordsapprove!toMonitorinfoDataAudit.action?projectid="+projectid+"&entid="+entid, 
		fit:true,
		fitColumns : true,
		scrollbarSize:0,
		remoteSort: false,
...
//					 ]],
		columns:[[
					{field:'attributetype',title:'监测点类型', width:80,align:"center"},
					{field:'status',title:'状态 ', width:70,align:"center"},
					{field:'pointname',title:'监测点名称 ', width:140,align:"center"},
					{field:'pointcode',title:'监测点编码', width:90,align:"center"},
					{field:'deptname',title:'采样科室 ', width:100,align:"center"},
...
//					{field:'plansamplingdate',title:'计划采样开始日期 ', width:100,align:"center"}
			]],
...
function initSampleinfo(projectcode,monitorpointid,flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath +'/sampletest/originalrecordsapprove/originalrecordsapprove!toSampleDataAudit.action?projectcode='+encodeURI(projectcode)+'&monitorpointid='+encodeURI(monitorpointid);
	}
	$('#samplelist').datagrid({
		nowrap: false,
		striped: true,
...
function showDataSourceDialog(){
	...
	if(deviceid!=""){
		var url = rootPath + "/sampletest/originalrecordsapprove/originalrecordsapprove!toShuJuZhuiSuList.action?samplecode="+samplecode+"&deviceid="+deviceid;
...
function dataAudit(){
	...
	if(samplecode!=""){
		var url = rootPath + "/sampletest/originalrecordsapprove/originalrecordsapprove!toDataAudit.action";
...
function initIteminfo(projectcode,monitorpointid,samplecode,flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath +'/task/sampleitem!toItemfoParameterAudit2.action?samplecode='+samplecode+'&projectcode='+projectcode+'&monitorpointid='+monitorpointid;
	}
	$('#itemlist').datagrid({
		nowrap: false,
		striped: true,
...
function checkFuntion(temp){	//原始数据签发（按任务签发）
...
function operate(temp,message){
	...
	//return;
	$.post(
		rootPath + "/sampletest/sample/sampletestbysample!setAllStatusByAudit.action",//"/sampletest/originalrecordsapprove/originalrecordsapprove!setAllStatusByAudit.action",	
...
//	if(deviceid!=""){
	if(batchno!=''){
		url = rootPath + "/sampletest/batch/batch!toShuJuZhuiSuList.action?batchno="+batchno+"&deviceid="+deviceid+"&itemid="+itemid;
	}else{
		url = rootPath + "/sampletest/originalrecordsapprove/originalrecordsapprove!toShuJuZhuiSuList.action?samplecode="+samplecode+"&deviceid="+deviceid;
	}
		var _dialog =  window.top.$('<div id ="ds-dlg" style="padding:0px;"><iframe id="dataSourceFrame" width="100%" height="100%" frameborder="0" scrolling="no" src='+url+'></iframe></div>').appendTo(window.top.document.body);
		_dialog.dialog({
			title:'数据追溯',
