...
function initDataGrid(){
			$('#datagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'sampletestbysample!fileList.action?samplecode='+samplecode,
				idField:'id',
				fit:true,
				fitColumns:true,
				scrollbarSize:0,
...
//					{field:'dossierno',title:'档案号',width:100,align : 'center'},
					{field:'operate',title:'操作',width:50,align : 'center',
						formatter:function(value,rowData,rowIndex){
							...
							return '<img src="'+rootPath+'/themes/default/images/deleteimage.png" alt="删除" onclick="deleteVersion(\''+rowData.samplecode+ '\',\''+rowData.filename+'\')"/>';
						}
					}					
				]]
//				pagination:true,
...
//添加和修改
function editVersion(samplecode){
//	$('#datagrid').datagrid('clearSelections');
//	var batchrows = $("#batchdatagrid").datagrid("getSelections");
	var url = rootPath + "/sampletest/sample/sampletestbysample!toFileInput.action";
	if(samplecode!=""){
	...
	buttons:[{
		...
		handler:function(){
			$("#fileFrame",top.document.body).contents().find("#fileForm").form('submit',
				{
					url:rootPath + '/sampletest/sample/sampletestbysample!saveFile.action?samplecode='+samplecode,
					onSubmit:function(){
...
//删除
function deleteVersion(samplecode,filename){
//	alert(samplecode+"---"+filename);
	if (samplecode!=null && samplecode!=""){
 		if(window.confirm('是否删除？'))
 		{
			$.post(
				rootPath + "/sampletest/sample/sampletestbysample!deleteFileVersion.action",
				{"samplecode":samplecode,"filename":filename},
				function(msg){
					if(msg=='success'){
						$('#datagrid').datagrid('clearSelections');
						alert("删除成功！");
...
//	var name=encodeURI(filename.toString());
	$.ajax({
		type: "POST",
		url: rootPath +"/sampletest/sample/sampletestbysample!downLoadImpl.action",
		...
		success:function(data){
			if(data=="fail"){
				...
				var urlParmDown=rootPath +"/sampletest/sample/sampletestbysample!downLoadImpl.action?path="+encodeURI(name)+"&flg=1";
