	...
	$('#tt').tabs({
		onSelect: function(title){
			if(title == '数据审核'){
				...
				if(batchno!=""){					
					var url = rootPath + "/sampletest/sampletestbybatch!toDataAudit.action";
	...
	$('#datagrid').datagrid(
		{
			...
			striped : true,
			collapsible : true,
			url : 'sampletestbybatch!dataAuditList.action', 
			sortName : 'itemid',
			sortOrder : 'asc',
			remoteSort : false,						
			fit : true,
...
function seldatagrid(itemid,batchno,flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath+'/sampletest/sampletestbybatch!selDataAuditList.action?itemid='+itemid+"&batchno="+batchno;
	}
//	if(batchno!=''){
