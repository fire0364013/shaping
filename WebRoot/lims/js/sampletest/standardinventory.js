...
function initDataGrid(flagEncodeName) { 
	...
	onSelect:function(){
		...
		checkRadio();
		$("#materialnameSel").val(selected['materialname']); //物品名称
		$("#inventoryidSel").val(selected['inventoryid']); //物品编号
		$("#batchnoSel").val(selected['batchno']);//批号
		$("#consistenceSel").val(selected['consistence']);
		$("#uncertaintySel").val(selected['uncertainty']);//不确定度
...
function showName(){
	...
	var flagNameReport=$("#reportitemname").val(); 
	var encodeName="";  
	if(name==""){//当没有选择项目就选择物质的时候，
		if(flagNameReport!=""){//当没有选择项目就选择物质的时候， 
			encodeName=encodeURI(encodeURI(flagNameReport));//当没有校准曲线对应的时候，查询相关物质。
		} 
	}else{
		encodeName=encodeURI(encodeURI(name));
	}
	var url =  rootPath +"/sampletest/sampletestbybatch!toShowName.action?flagItemname="+encodeName;
