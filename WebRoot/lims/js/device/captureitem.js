			...
			.datagrid(
					{
						...
						striped : true,
						collapsible : true,
						url : rootPath +'/device/capture/captureitemmap!captureList.action?deviceid='+deviceid,
						sortName : 'id',
						sortOrder : 'asc',
						remoteSort : false,
						//idField : 'id',
...
function addWin(id) {
	...
	var url =rootPath+"/device/capture/captureitemmap!input.action";
	if (id != "") {
		url = url + "?id=" + id;
	}
		...
		buttons : [
				{
					...
					iconCls : 'icon-save',
						...
						$("#captureFrame",top.document.body).contents().find("#captureform").form('submit',{
								url:rootPath +'/device/capture/captureitemmap!savecapture.action?id='+id,
								onSubmit:function(){
								},
...
//	$('#captureitem').datagrid('selectRecord',uid);
	if (window.confirm('是否删除？')) {
		$.post(rootPath+"/device/capture/captureitemmap!deleteOnlyOne.action", {id : uid}, function(del) {
			if (del == 'success') {
				alert("删除成功！");
				$("#captureitem").datagrid('reload');
			}else{
...
function delAll() {
	...
	if (selected != null && selected != "") {
		if (window.confirm("是否删除？")) {
			...
			$.post(rootPath+"/device/capture/captureitemmap!betchDeleteMethod.action?id="
...
function addCurveDialog(){
	var url = rootPath+"/device/capture/captureitemmap!tocaptureitemInput.action?deviceid="+$("#deviceid").val();
		...
		buttons:[{
			...
			handler:function(){
				$("#captureFrame",top.document.body).contents().find("#curveform").form('submit',{
						url:rootPath +'/device/capture/captureitemmap!savecapture.action',
						onSubmit:function(){
...
//未过滤了的~~begin ================================================================
		...
		function methodDataSingle(itemid){
			...
			$.ajax( {
				type : 'GET',
				url : rootPath+'/device/capture/captureitemmap!initMethod.action?'+'timeStamp='+new Date().getTime(),//给该url一个时间戳~~这样就必须每次从服务器读取数据,
				data : {'itemid':itemid},
				async:false,//同步
				success : function(data) {
					var vData = eval("(" + data + ")");
