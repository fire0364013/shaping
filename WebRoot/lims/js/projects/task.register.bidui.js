				...
				if($('#projectcode').val()!=''){
					if($('#BasicInfoFrame').attr('src')==undefined||$('#BasicInfoFrame').attr('src')==""){
						if($('#stepcode').val()!='Register'&& $('#stepcode').val()!='TaskApprove'){
							...
							$("#BasicInfoFrame").attr("src",rootPath +"/projects/taskregister/taskregister!biduiInput.action?id="+$('#projectcode').val()+"&timeStamp="+new Date().getTime()+"&monitortype=4");
						}
					}
				}	
...
//									}else if (data == '2'){
//										$("#TaskListFrame").attr("src",rootPath + "/common/report!toReportPage.action?raq="+encodeURI(encodeURI("例行监测任务计划单（悬浮物中重金属）.raq"))+"&projectcode="+$('#projectcode').val());
//									}else{
//										alert(data);
//									}
//					
						...
						$.ajax( {
								...
								success : function(data) {
									...
									if (data == 'fail') {
										...
										$("#WeituoFrame").attr("src",rootPath + "/common/report!toReportPage.action?raq="+encodeURI(encodeURI("例行监测任务计划单（环境空气质量监测）.raq"))+"&projectcode="+$('#projectcode').val());
									}else{
										alert(data);
									}
...
function toTaskBasicInfo(){
	var url = rootPath + "/projects/taskregister/taskregister!biduiInfo.action?monitortype=4";
	...
	$("#MonitorForm").form('submit',{
			...
			success : function(data) {
				if (data == 'fail') {
					...
					alert('成功！');
					//initDataGrid();
//					$('#datagrid',top.document.body).treegrid('reload');
...
function toSearchObj(){
	var url = rootPath + "/projects/taskregister/taskregister!toSearchPage.action?monitortype=4";
...
function checkFuntion(action){	
	var projectcode = $("#projectcode").val();
	var url = rootPath + "/projects/appraiseopinion/appraiseopinion!opinionNew.action?showName="+encodeURI(encodeURI(action+"意见"))+"&projectcode="+projectcode+"&scode="+$('#stepcode').val();
...
//	_dialog.dialog('open');
//}
function initWorkflow(projectcode){
	if(window.confirm('是否提交？')){
