				...
				if($('#projectcode').val()!=''){
					if($('#BasicInfoFrame').attr('src')==undefined||$('#BasicInfoFrame').attr('src')==""){
						if($('#stepcode').val()!='Register'&& $('#stepcode').val()!='TaskApprove'){
							...
							$("#BasicInfoFrame").attr("src",rootPath +"/projects/taskregister/taskregister!linshiInput.action?id="+$('#projectcode').val()+"&timeStamp="+new Date().getTime()+"&monitortype=3");
						}
					}
				}	
						...
						success : function(data) {
							if(data=="1"){
								if($('#SubFrame').attr('src')==undefined||$('#SubFrame').attr('src')==""){
									$("#SubFrame").attr("src",rootPath +"/projects/taskregister/taskregister!toSubPage.action?projectcode="+$('#projectcode').val()+"&timeStamp="+new Date().getTime());
								}
							}else {
								alert("此项目不是分包项目！");
							}
						}
//					alert($('#issubbackage').val());
...
function toTaskBasicInfo(){
	var url = rootPath + "/projects/taskregister/taskregister!linshiInfo.action?monitortype=3";
...
function toSearchObj(){
	var url = rootPath + "/projects/taskregister/taskregister!toSearchPage.action?monitortype=3";
