function initDataGrid(){
	$('#datagrid').treegrid({
		...
		striped: true,
		collapsible:true,
		url:'taskregister!yiyuantaskList.action',
		queryParams : {
			monitortype : 7
		},
		fit:true,
...
//		 	},
		...
		columns:[[
	        {field:'stepname',title:'项目状态',width:80,align : 'center'},	        
	...
	$('#tt').tabs({
		onSelect: function(title){
			if(title == '流程图'){
				...
				if($('#customid').val()!=''){
					if($('#CustomFrame').attr('src')==undefined||$('#CustomFrame').attr('src')==""){
						//if($('#stepcode').val()!='Register'){
							$("#CustomFrame").attr("src",rootPath +"/custom/custom!view.action?id="+$('#customid').val());
						//}else{
						//	$("#ChargeFrame").attr("src",rootPath +"/projects/taskregister/chargedetail!list.action?id="+$('#projectcode').val());
						//}
	...
	_dialog.dialog({
		...
		buttons:[{
			...
			handler:function(){
					...
					var employeeName=$("#employeeInfoFrame",top.document.body).contents().find("#name").val();
					var employeephone=$("#employeeInfoFrame",top.document.body).contents().find("#phone").val();
					$("#counselorname").val(employeeName);
					$("#counselorid").val(employeeId);
					$("#counselorphone").val(employeephone);
//					$("#projectname").val(entName);
					_dialog.dialog('close');
				}
			},{
...
function toSearchObj(){
	var url = rootPath + "/projects/taskregister/taskregister!toyiyuanSearchPage.action?monitortype=7";
