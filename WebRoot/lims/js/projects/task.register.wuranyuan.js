				...
				if($('#projectcode').val()!=''){
					if($('#BasicInfoFrame').attr('src')==undefined||$('#BasicInfoFrame').attr('src')==""){
						if($('#stepcode').val()!='Register'&& $('#stepcode').val()!='TaskApprove'){
							...
							$("#BasicInfoFrame").attr("src",rootPath +"/projects/taskregister/taskregister!wuranyuanInput.action?id="+$('#projectcode').val()+"&timeStamp="+new Date().getTime()+"&monitortype=1");
						}
					}
				}				
			}else if(title == '任务计划单'){
...
function toTaskBasicInfo(){
	var url = rootPath + "/projects/taskregister/taskregister!wuranyuanInfo.action?monitortype=1";
...
{
	var url = rootPath + "/projects/taskregister/taskregister!toEntpriseListPage2.action?monitortypeid=1";
...
function toSearchObj(){
	var url = rootPath + "/projects/taskregister/taskregister!toSearchPage.action?monitortype=1";
