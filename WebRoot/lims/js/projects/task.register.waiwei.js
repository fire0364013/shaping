				...
				if($('#projectcode').val()!=''){
					if($('#BasicInfoFrame').attr('src')==undefined||$('#BasicInfoFrame').attr('src')==""){
						if($('#stepcode').val()!='Register'){
							...
							$("#BasicInfoFrame").attr("src",rootPath +"/projects/taskregister/taskregister!waiweiInput.action?id="+$('#projectcode').val()+"&timeStamp="+new Date().getTime()+"&monitortype=7");
						}
					}
				}	
...
function toSearchObj(){
	var url = rootPath + "/projects/taskregister/taskregister!toSearchPage.action?monitortype=7";
