...
function initDataGridtree (){
		...
		$('#datagrid').tree({
			url: rootPath+"/spot/samplingdataaudit/originaent!sampleitemListnew.action?projectcode="+pid,
			...
			$("#del").show();
				...
				if(dnode!=null){
						...
						if(itemstr=='entid'){
							entid = node.id;
							flagUrl=rootPath+'/spot/samplingdataaudit/originaent!sampleitemListMethod.action?projectcode='+pid+'&entid='+entid;
						}else if(itemstr=='lx'){
							lxid = node.id;
							var parentnode=$('#grouptree').tree('getParent',node.target);
							entid =parentnode.id;
							flagUrl=rootPath+'/spot/samplingdataaudit/originaent!sampleitemListMethod.action?projectcode='+pid+'&entid='+entid+'&lx='+lxid;
						}else if(itemstr=='rq'){
							var parentnode=$('#grouptree').tree('getParent',node.target);
							lxid =parentnode.id;
							...
							rq=node.id;
							flagUrl=rootPath+'/spot/samplingdataaudit/originaent!sampleitemListMethod.action?projectcode='+pid+'&entid='+entid+'&lx='+lxid+'&cydate='+rq;
						}
...
function unSeldatagrid(itemidVal,flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath+'/spot/samplingdataaudit/originaent!unSelSampleitemList.action?itemid='+itemidVal;
	}
...
function seldatagrid(itemidVal,batchno,flag,star) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath+'/spot/samplingdataaudit/originaent!selSampleitemList.action?itemid='+star+"&batchno="+batchno;
	}
...
//				}else{
					...
					$.post(
						rootPath + "/spot/samplingdataaudit/originaent!addParallelSample.action"+ url, function(data) {
						if (data == 'success') {
							alert("添加成功！");
							$("#seldatagrid").datagrid('reload');
//							$("#seldatagrid").datagrid("clearSelections");
...
function methodData(itemid,index,pid,methodid,cellvalue,mpid,mptid,cydate,entid) {
		$.ajax( {
			type : 'GET',
			url : rootPath+'/spot/samplingdataaudit/originaent!getMethod.action?itemids='+itemid+'&timeStamp='+new Date().getTime(),//给该url一个时间戳~~这样就必须每次从服务器读取数据,
			async:false,//同步
			success : function(data) {
				var vData = eval("(" + data + ")");
				...
				$("#methodSel"+index).change(function(){
					...
					$.ajax( {
						type : 'GET',
						url :rootPath+'/spot/samplingdataaudit/originaent!updateMethod.action?itemids='+itemid+'&methodid='+mid+'&pid='+pid+'&mpid='+mpid+'&mptid='+mptid+'&cydate='+cydate+'&entid='+entid+'&timeStamp='+new Date().getTime(),//给该url一个时间戳~~这样就必须每次从服务器读取数据,
						async:false,//同步
						success : function(data) {
							if(data=="success"){
//								alert("修改成功！");
...
function toentydata(pid,entid,lxid,rq){   
    $.ajax( {
		type : 'POST',
		url : rootPath+"/spot/samplingdataaudit/originaent!getStr.action?pid="+pid+"&entid="+entid+"&lxid="+lxid+"&rq="+rq,
		async:false,//同步
		success : function(data) { 
	    	var vData = eval("(" + data + ")");
...
function savedata(){
	...
	$.post(rootPath+"/spot/samplingdataaudit/originaent!getSavedata.action",
...
function totj(){
	...
	$.post(rootPath+"/spot/samplingdataaudit/originaent!getSampletestresultSize.action",{pid:pid,mpid:mpid},
						function(msg){
							if(msg == 'yes')
							{
...
function operate(pid,mpid,entid,userid,actionname){
	...
	$.post(rootPath+"/spot/samplingdataaudit/originaent!setAllStatus.action",{pid:pid,mpid:mpid,userid:userid,entid:entid,actionname:actionname},				
		function(msg){
			if(msg=='success'){
				jQuery.unblockUI();
				alert("提交成功！");
