...
function initDataGrid(){
	$('#datagrid').datagrid({
		...
		striped: true,
		collapsible:true,
		url:rootPath + '/spot/samplingdatarecheck/samplingdatarecheck!taskList.action?stauts='+$("#taskStatus").val(),
		fit:true,
		border:true,
		fitColumns:false,
		scrollbarSize:0,
...
function initMonitorinfo(){	
	$('#monitorinfolist').datagrid({
		...
		striped: true,
		collapsible:true,
		url: rootPath + "/spot/samplingdatarecheck/samplingdatarecheck!detailList.action?projectcode="+projectcode,
		fit:true,
		fitColumns:true,
		scrollbarSize:0,
...
function initSamplsData(projectcode,monitorpointid,entid){
	$('#sampleslist').datagrid({
		...
		striped: true,
		collapsible:true,
		url:rootPath +'/spot/samplingdatarecheck/samplingdatarecheck!initSamplesList.action?projectcode='+encodeURI(projectcode)+'&monitorpointid='+encodeURI(monitorpointid)
		+'&entid='+entid, 
		fit : true,
		scrollbarSize : 0,
		remoteSort:false,
...
function toSearchObj(){
	var url = rootPath + "/spot/samplingdatarecheck/samplingdatarecheck!toSearchPage.action";
...
function entryoriginal(){
	...
	if(selected!=null){
		if(selected.projectcode!=""){
			...
			var url =  rootPath +"/spot/samplingdatarecheck/originaent!toSampletestbybatchList.action";
			var surl =  rootPath +"/monitor/monitorpoint!update.action";
			if(id!=""){
				url = url + "?id="+id;
			}
...
////				alert(taskid);
//				dataCommit(projectcode,'LocalDataRecheck',"提交",taskid,auditperson,'');
//				_dialog.dialog('close');
//			}
//		},{
//			text:'取消',
...
//						}
			...
			dataCommit(projectcode,'LocalDataRecheck',action,taskid,userid,opinion);
			_dialog.dialog('close');
			}
		},{
		text:'取消',
...
function initWorkflow(){
	...
	$.ajax( {
		...
		data: "status=LocalDataRecheck&workflowcode=JC_PROJECT",
		success : function(data) {
			var vData = eval("(" + data + ")");
			var lList = "";
			jQuery.each(vData, function(i, n) {
...
function initRecheckPersion(){
	$.ajax( {
		...
		success : function(data) {
			...
			$('#auditperson',window.top.document).html(lList);
		}
	});
}
//点击提交按钮
//function recheckOperate(action){
//	var rows=$("#sampleslist").datagrid('getSelections');
//	var projectcode = $("#projectcode").val();
//	if(rows==""){
//		alert("请至少选择一条记录！");
...
//				var auditperson=$(window.top.document).find("#userid").val();
////				alert(taskid);
//				dataCommit(projectcode,'LocalDataRecheck',"提交",'',auditperson,'');
//				_dialog.dialog('close');
//			}
//		},{
//			text:'取消',
...
//				return false;
//			}
//			dataCommit(projectcode,'LocalDataRecheck','退回','',userid,message);
//			
//			_dialog.dialog('close');
//			}
//		},{
