...
function initDataGridtree (){
		...
		$('#datagrid').tree({
			url: rootPath+"/spot/samplingdata/originaent!sampleitemListnew.action?projectcode="+pid,
			...
			$("#del").show();
				...
				if(dnode!=null){
						...
						if(itemstr=='entid'){
							entid = node.id;
							flagUrl=rootPath+'/spot/samplingdata/originaent!sampleitemListMethod.action?projectcode='+pid+'&entid='+entid;
						}else if(itemstr=='lx'){
							lxid = node.id;
							var parentnode=$('#grouptree').tree('getParent',node.target);
							entid =parentnode.id;
							flagUrl=rootPath+'/spot/samplingdata/originaent!sampleitemListMethod.action?projectcode='+pid+'&entid='+entid+'&lx='+lxid;
						}else if(itemstr=='rq'){
							var parentnode=$('#grouptree').tree('getParent',node.target);
							lxid =parentnode.id;
							...
							rq=node.id;
							flagUrl=rootPath+'/spot/samplingdata/originaent!sampleitemListMethod.action?projectcode='+pid+'&entid='+entid+'&lx='+lxid+'&cydate='+rq;
						}
...
function unSeldatagrid(itemidVal,flag) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath+'/spot/samplingdata/originaent!unSelSampleitemList.action?itemid='+itemidVal;
	}
...
function seldatagrid(itemidVal,batchno,flag,star) {
	...
	if(flag=='flag'){ //此处是加载的时候不查询后台，但是要把头儿展示出来。2012-04-11
		...
		flagUrl=rootPath+'/spot/samplingdata/originaent!selSampleitemList.action?itemid='+star+"&batchno="+batchno;
	}
...
//		}*/
		...
		$.post(
			rootPath + "/spot/samplingdata/originaent!getDevicecalibratecurve.action"+ url, function(msg){
			if (msg == 'yes') {
				if(sampleitemtestids!=""){
					if(confirm("是否使用上次校准曲线？")){
						url = "?batchno="+batchno+"&sampleitemtestid="+sampleitemtestids;
						$.post(
							rootPath + "/spot/samplingdata/originaent!addBatchno.action"+ url, function(msg){
							if (msg == 'success') {
								//alert("选择成功!");
								$("#unseldatagrid").datagrid('reload');
								$("#seldatagrid").datagrid('reload');
...
function addToLeftList(){
	...
	if(batchno!=""){
		if(sampleitemtestids!=""){
			...
			$.post(
				rootPath + "/spot/samplingdata/originaent!delBatchno.action"+ url, function(del){
				if (del == 'success') {
					//alert("选择成功!");
					$("#unseldatagrid").datagrid('reload');
					$("#seldatagrid").datagrid('reload');
...
//				}else{
					...
					$.post(
						rootPath + "/spot/samplingdata/originaent!addParallelSample.action"+ url, function(data) {
						if (data == 'success') {
							alert("添加成功！");
							$("#seldatagrid").datagrid('reload');
//							$("#seldatagrid").datagrid("clearSelections");
...
//				},
			...
			onDblClickCell:function(rowIndex, field, value){
				...
				$('#batchdatagrid').datagrid('selectRow',rowIndex);
				...
				if(field =="itemmethod" && row.itemid!=""){	
					...
					methodData(row.itemid,rowIndex,row.pid,row.methodid,value,row.mpid,row.mptid,row.cydate,row.entid);
					indexrow = rowIndex;
					indexvalue = value;
					...
					samplingFeeData(row.itemid,rowIndex,row.pid,row.methodid,value,row.mpid,row.mptid,row.cydate,row.entid);
					indexrow = rowIndex;
					indexvalue = value;
				}
			}
...
function methodData(itemid,index,pid,methodid,cellvalue,mpid,mptid,cydate,entid) {
		$.ajax( {
			type : 'GET',
			url : rootPath+'/spot/samplingdata/originaent!getMethod.action?itemids='+itemid+'&timeStamp='+new Date().getTime(),//给该url一个时间戳~~这样就必须每次从服务器读取数据,
			async:false,//同步
			success : function(data) {
				var vData = eval("(" + data + ")");
				...
				$("#methodSel"+index).change(function(){
					...
					$.ajax( {
						type : 'GET',
						url :rootPath+'/spot/samplingdata/originaent!updateMethod.action?itemids='+itemid+'&methodid='+mid+'&pid='+pid+'&mpid='+mpid+'&mptid='+mptid+'&cydate='+cydate+'&entid='+entid+'&timeStamp='+new Date().getTime(),//给该url一个时间戳~~这样就必须每次从服务器读取数据,
						async:false,//同步
						success : function(data) {
							if(data=="success"){
//								alert("修改成功！");
...
function toentydata(pid,entid,lxid,rq){   
    $.ajax( {
		type : 'POST',
		url : rootPath+"/spot/samplingdata/originaent!getStr.action?pid="+pid+"&entid="+entid+"&lxid="+lxid+"&rq="+rq,
		async:false,//同步
		success : function(data) { 
	    	var vData = eval("(" + data + ")");
...
function savedata(){
	var rows = $('#day_health').datagrid('getRows');
	for(var i=0;i<rows.length;i++){
		
	}
	var canitem = $("#canitem").val();
	alert(canitem);
	$.post(rootPath+"/spot/samplingdata/originaent!getSavedata.action",
		{kk:canitem},
		function(data) { 
	    	alert('完成'); 
    });
