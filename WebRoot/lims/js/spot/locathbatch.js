...
function initBatchDataGrid(itemid,flag,star) {
	var flagUrl;
...
//				},
			...
			onLoadSuccess:function(){
				...
				if(rowData!=null){
					seldatagrid(itemid,rowData.batchno,'',star);
				}else{
					seldatagrid('','','');
				}
...
//	var isTrue=false;
	...
	//alert(batchno);
	if(batchno!=""){
		$.post(rootPath + "/spot/locathbatch/locathbatch!getSampleitemSizeByBatchno.action?batchno="+ batchno,
			function(msg){
			if(msg == 'success')
			{	
...
//					}else if(msg=='data'||msg=='nopreitem'){//有前置项目有数据或无前置项目//maximize:yes;最大化   minimize:yes  最小化
						var url = rootPath + "/spot/samplingdata/originaent!toDataEntry.action";
