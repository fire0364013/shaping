...
function addParallel(){ 
	...
	if(sampleitemtestids!=""){ 
		sampleitemtestids = sampleitemtestids.substring(0,sampleitemtestids.length-1);
	 	$.post(
			rootPath + "/spot/samplingdata/originaent!addParallel.action?sampleitemtestids="+sampleitemtestids, function(data) {
			if (data == 'success') {
				alert("添加成功!");
				initHtml();
			}else{
...
function initHtml(msgDiv){//alert("原始数据");
	if(msgDiv!='existLoadDiv'){
		...
		$.ajax({
			type: "post",
			url: rootPath+"/spot/samplingdata/originaent!dataEntry.action?itemid="+itemid+"&batchno="+batchno+"&" + new Date().getTime(),
			success: function(msg){
				//alert(msg);
				//var obj = eval('('+msg+')');
				$("#inner1").html('');
...
function showToolTip(flag,name){
   ...
   if(flag){
       ...
       my_tips.style.top=parseInt($("input[name='"+name+"']").offset().top)-32;
