	...
	function initDataGrid(){
		$('#typedatagrid').datagrid({
			...
			striped: true,
			collapsible:true,
			url:rootPath +"/monitorproject/externalenvparam/externalenvparamdata!getMoniterTypeTree.action?projectid="+projectid,
			fit:true,
			fitColumns : true,
			scrollbarSize:0,
			remoteSort: false,
	...
	function getParameterData(rowIndex,rowData){
		if(rowData!=null&&rowData!=""){
			...
			$.post(rootPath +"/monitorproject/externalenvparam/externalenvparamdata!getExterParTime.action",{projectcode:projectcode,monitorpointtypeid:rowData.monitorpointtype},function(dataTime){
				var objTime = eval('('+dataTime+')');
				$("#samplingdateAdd").html(objTime.htmlTime);
				var  samplingdate=$("#samplingdateAdd").val();
				//综合环境参数信息
				$.post(rootPath +"/monitorproject/externalenvparam/externalenvparamdata!initAddInputData.action",{projectcode:projectcode,monitorpointtypeid:rowData.monitorpointtype,samplingdate:samplingdate},function(data){
					var innerHtml = eval('('+data+')');
					$("#tableaddson").html('');
					$("#tableaddson").html(innerHtml.html);
					//样品环境参数信息
					$.post(rootPath +"/monitorproject/externalenvparam/externalenvparamdata!initSamplesData.action",{projectcode:projectcode,monitorpointtypeid:rowData.monitorpointtype,samplingdate:samplingdate},function(data){
						//var innerHtml = eval('('+data+')');
						$("#samplesDiv").html('')
	...
	function saveExtenParam	(){
		$("#extenParamForm").form('submit',{
			url:rootPath +"/monitorproject/externalenvparam/externalenvparamdata!saveAddData.action"
			//onSubmit: function(){
...
//					 alert("没有参数不能保存");
	...
	function saveSampleData(){
		$("#samplesForm").form('submit',{
			url:rootPath +"/monitorproject/externalenvparam/externalenvparamdata!saveSmaplesData.action"
			//onSubmit: function(){
